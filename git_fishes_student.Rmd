...
[My GitHub Repository](https://github.com/JadeFletcher/3207_workshop.git)
...
## If GitHub for whatever reason was down or you lose access to your account you would also lose whatever is stored on it.
...
# The code block will not be visible in the render.
```
...
```{r, summaryTab}
clean <- complete %>% select(-loc, -comment, -...1)

summary <- clean %>% group_by(species) %>%   summarise(across(c(sl, activity), funs(mean, se=sd(.)/sqrt(n()), n())))
...
## Green highlights are things added to files, red are deletions.
```
...
boxgraph <- ggplot(data = clean, mapping = aes(y = activity, x = species, fill = treatment)) + geom_boxplot()
...
```{r, savefig}
ggsave('graph7', boxgraph, path = 'output/figures/', device = 'png')
```
...
```{r}
# new graphs in this location do not appear in the repo.
```
...
```{r, answer6}
# you would want to track them so you are able to call on the images to create the graph. Otherwise the code would be calling on an empty or nonexistent folder to access the images.
```
...
clean[clean$species == c('chromis', 'lemon', 'acantho'), ]

ggplot(clean[clean$species == c('chromis', 'lemon', 'acantho'), ], aes(activity, sl, color=treatment)) + geom_boxplot() + facet_wrap(~species)
```
...
(for(type in unique(clean$species)){
 m <-  t.test(clean[clean$species == type & clean$treatment == 'CO2', 'activity',],
       clean[clean$species == type & clean$treatment == 'control', 'activity'], paired = FALSE)
 species <- type
 mean_diff <- m$estimate[1]-m$estimate[2]
 lower_confint <- m$conf.int[1]
 upper_confint <- m$conf.int[2]
 t_stat <- m$statistic
 df <- m$parameter
 p_value <- m$p.value
 
 q <- data.frame(species, mean_diff, lower_confint, upper_confint, t_stat, df, p_value)
 emp <- rbind(emp, q)
})
...
```{r, table1, echo=TRUE, eval=TRUE, tab.cap = "summary statistics for 6 t-tests on 6 species of fish between two treatment groups"}
emp
```
...
Mean activity for `r "humbug"` in the control group was `r mean((clean[clean$species == 'humbug' & clean$treatment == 'control', 'activity'])$activity)` (s / min) compared to the OA treatment group, which was `r mean((clean[clean$species == 'humbug' & clean$treatment == 'CO2', 'activity'])$activity)` (s / min). The difference between control and OA treatment means was `r mean((clean[clean$species == 'humbug' & clean$treatment == 'control', 'activity'])$activity) - mean((clean[clean$species == 'humbug' & clean$treatment == 'CO2', 'activity'])$activity)` (s / min) (95% CI: `r (emp[emp$species == 'humbug', ])$lower_confint` to `r (emp[emp$species == 'humbug', ])$upper_confint`). 
...
Null Hypothesis - There was no difference in the mean activity between the control and ocean acidification treatment group. Our p-value.

for the species humbug, the p-value observed was `r (emp[emp$species == 'humbug', ])$p_value`. This is below the generally accepted cutoff of p = 0.05, so based off of this test we can reject the null hypothesis. That is, we do observe a statistically significant difference in mean activity between treatments.
...
samp_dist <- tibble(permuted_p = rep(F,200), unusual = rep(F,200))

for(x in 1:200){
  newdata <- clean %>% mutate(treatment = sample(treatment, length(treatment), replace = F)) %>% filter(species == "humbug")
  samp_dist$stat[x] <- t.test(newdata$activity[newdata$treatment == "CO2"],newdata$activity[newdata$treatment == "control"])$statistic
  samp_dist$extreme[x] <- (abs(samp_dist$stat[x]) > abs(results_activity$T.stat[1]))
}

print(length(which(samp_dist$extreme)) / length(which(!samp_dist$extreme))) 
```
